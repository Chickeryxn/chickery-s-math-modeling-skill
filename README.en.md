...
## Modeling-mode reference sources

In `modeling` (non-training / formal modeling) mode, every generation step (parsing, classification, method screening, figures/tables, paper writing) **must treat these three groups as important references** (full list: `references/reference-sources.md`):

1. **All resource-library entries** — read `resource-library/index.json` first, then consult at least one entry in each relevant category (papers/ideas/figures/formulas/tables/assets);
2. **Prior CUMCM award papers** — `references/award-papers/` (from https://github.com/Chickeryxn/paper ; 2019–2024 award papers + 2019–2025 problems). Before writing the paper, compare its **content arrangement, paper format, and capacity**, ensure the paper contains all contest-required **figures/tables**, and produce a **PDF compiled from LaTeX**;
3. **Configured upstream links** — [Yuan1z0825/nature-skills](https://github.com/Yuan1z0825/nature-skills), [jihe520/sci-box](https://github.com/jihe520/sci-box) (plus Lupynow / CUMCMThesis / XiaoMaColtAI); nature-skills and sci-box are mandatory.

`training` (closed-book) mode forbids both of the first two groups until the modeler explicitly opts into open-book comparison. See `references/reference-sources.md` and `references/award-papers/README.md`.
