...
- **papers/**: 2 per problem `20XX{A,B,C}_paper_{note,structure}` — multi-paper consensus (highlights / modeling-decision chain / results presentation / literacy mapping / limitations & counterexamples) and paper-structure reflection (overall + per-section), flagging counterexamples (inconsistent units/symbols, model-vs-code-vs-text conflicts, claimed-but-unmet constraints, contradictory results, missing cross-validation/sensitivity, unseeded randomness).
- **ideas/**: 2 per problem (idea cards: object→variables→relations→constraints/goal + quantified risk + lesson + transferability).
- **formulas/**: 2 per problem (key formulas + symbols/premises + derivation + numeric example + pitfalls + transferability), each with a reusable `content/<id>.tex` LaTeX artifact.
- **tables/**: 1 per problem (booktabs three-line result/comparison/sensitivity skeleton + transferable table rules), each with a reusable `content/<id>.tex` booktabs template.
- Abstract literacy covered: coordinate transforms / geometric-symmetry dimension reduction, cheap-proxy → exact-verify, scale reduction (magnitude argument PDE→0D), mechanism + data identification, invert-then-control, hard-constraints-as-a-feasibility-checker, prove an invariant/homogeneity before optimizing, scale-led paradigm choice (enumeration→DP→heuristics), log-ratio transforms for compositional data, data-shape→method→algorithm mapping, strip confounders before estimating effects, objective weighting + human calibration, unit-normalization (per-unit-time net benefit), expected-value vs risk measures (expectation/CVaR/three-criteria/robust), unified measurement conversion, multi-objective priority/Pareto, predict-then-optimize decoupling, external shocks as multipliers, system simulation + state experiments + one-factor perturbation.
- **figures/**: the previously local-only exemplars (`figures/exemplar-2025A/B/C`, `figures/topjournal-style/`) were added to version control and registered in `index.json` (figures now = 6 entries).
- Source material: `https://github.com/Chickeryxn/paper` (user-owned repo); the library stores only distilled notes — no copied problems or full papers, with per-entry source/rights.
- Version alignment: both plugin manifests + marketplace + README badges + CHANGELOG bumped to **0.10.0**.
- Docs: `docs/review/` refreshed to 0.10.0 (added a 0.10.0 re-review addendum covering the Problem-Start Mode Gate and the resource-library expansion; unified the current-test-count wording).
- Notes: `resource-library/index.json` is machine-generated (`python scripts/resource_index.py .`) and shipped consistent with the committed tree.
...
- Version badges/plugin manifests/marketplace bumped to 0.9.0; tests grew to 247.
