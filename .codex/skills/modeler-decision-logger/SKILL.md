...
The `user_message_id` is the runtime's stable message identifier. Under DeepSeek Harness use the convention `dsh:<session_id>:<seq>` where `<session_id>` comes from `$env:DSH_SESSION_ID` and `<seq>` is a per-session counter; under Codex/Claude use the platform message id. The validator only requires a non-empty string.
