...
def run(root,run_dir,args):
 ...
 started=time.time()
 try:
  proc=subprocess.run(args.command,cwd=root,shell=True,text=True,capture_output=True)
...
def validate(root,run_dir):
 ...
 if bool(d.get('budget_delta'))!=bool(d.get('degraded')):e.append('budget_delta/degraded mismatch')
...
def main():
 ...
 try:
  if a.mode=='begin':out=begin(root,rd,a)
  elif a.mode=='run':out=run(root,rd,a)
  elif a.mode=='finalize':out=finish(root,rd,a.status,a.result_ref,a.validation_ref,a.return_code,False)
  else:out=validate(root,rd)
