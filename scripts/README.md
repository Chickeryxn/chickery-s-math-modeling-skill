...
- `check_frozen_freshness.py`: per-claim freshness of `frozen_numbers.json` (source exists, not newer than `frozen_at`); exit 2 on any stale claim. Freshness is mtime-based: a git checkout, unpack, or copy that resets mtimes can produce a false STALE verdict — treat STALE as "verify and re-freeze", not as proof of tampering. Naive `frozen_at` values are interpreted as UTC and raise an advisory warning (use an explicit offset). Wired into `validate_repo.py` (runs when a `frozen_numbers.json` exists).
