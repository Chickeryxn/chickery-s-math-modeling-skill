...
def main():
 ...
 if not isinstance(d,dict):
  print('CONTRACT_INVALID: top-level JSON must be an object',file=sys.stderr);return 2
 ...
 obj=as_dict(d.get('objective'));ev=as_dict(d.get('evaluator'));vc=as_dict(d.get('validation_contract'))
 ...
 if a.require_resolved:
  ...
  placeholders=('problem-specific','todo:','placeholder','待填','占位','example only','lorem')
  if any(p in raw for p in placeholders):errors.append('resolved contract still contains placeholder/example text')
