...
Project-relative `evidence_refs` must resolve to files inside `PROJECT_ROOT` (the workspace root; under DeepSeek Harness this is the opened repository); escaping paths and missing files are invalid. `DECIDED` requires `source.source_type=user_answer`, `user_message_id`, and `user_verbatim_answer`. `SUPERSEDED` and `STALE` require a valid earlier `supersedes` record. Run `scripts/validate_decisions.py` before updating the manifest.

The `user_message_id` is the runtime's stable message identifier. Under DeepSeek Harness use the convention `dsh:<session_id>:<seq>` where `<session_id>` comes from `$env:DSH_SESSION_ID` and `<seq>` is a per-session counter; under Codex/Claude use the platform message id. When the runtime provides no stable message identifier, record the explicit marker `unavailable:<platform>` (e.g. `unavailable:codex`) together with the verbatim answer — never invent a fake id. The validator only requires a non-empty string.
