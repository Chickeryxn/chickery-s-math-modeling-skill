...
- **变体轮**：多轮训练应尽量换题/换数据（逐轮指定题源），或在 clean 轮要求 agent 同时禁读上一轮 `results/training/roundN-1/solution/`——`training-solver` 已声明该规则，防止"记住上一轮答案"式虚假提升。
- **平台边界**：仓库 hooks 只有 PreToolUse **写**拦截（冻结数字/原始数据）；闭卷隔离没有跨平台的**读**拦截钩子，`closed` 约束以 `training_config` 双声明 + 人工监督为准（见 `docs/dsh-compatibility.md` §五）。
